...
// Taken and modified from github.com/fluffle/goirc/client/dispatch.go
...
type HandlerFunc func(*Module, *Line)

func (hf HandlerFunc) Handle(module *Module, line *Line) {
	hf(module, line)
}
...
func (hn *hNode) Handle(module *Module, line *Line) {
	hn.handler.Handle(module, line)
}
...
func (hs *hSet) dispatch(module *Module, line *Line) {
	hs.RLock()
	defer hs.RUnlock()
	ev := strings.ToLower(line.Line.Cmd)
	list, ok := hs.set[ev]
	if !ok {
		return
	}
...
func (module *Module) Handle(name string, h Handler) Remover {
	return module.handlers.add(name, h)
}

// HandleFunc adds the provided function as a handler in the foreground set
// for the named event.
// It will return a Remover that allows that handler to be removed again.
func (module *Module) HandleFunc(name string, hf HandlerFunc) Remover {
	return module.Handle(name, hf)
}

// func (module *Conn) dispatch(line *Line) {
...
// func (module *Module) LogPanic(line *Line) {
